...
//---------------------------------------------------------------------------------------------------------------------------------
OrcBerserkMobAI::OrcBerserkMobAI():
UnitAI(),
initialize(false),
waitAnimation(0),
raiseDamage(0)
{
    typeUnitAI = ORC_BERSERK_MOB_AI;
}
...
//---------------------------------------------------------------------------------------------------------------------------------
OrcBerserkMobAI* OrcBerserkMobAI::createTS(BattleModel* _battleModel)
{
    OrcBerserkMobAI* pRet = new OrcBerserkMobAI();
    if(pRet)
...
//---------------------------------------------------------------------------------------------------------------------------------
void OrcBerserkMobAI::init(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits)
{
    /* here a initialize default parameters
    */

    raiseDamage = _unitAI->getAdditionalParameter(1);    //rise of damage in percent

    initialize = true;
}

//---------------------------------------------------------------------------------------------------------------------------------
void OrcBerserkMobAI::update(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits, float _dt)
{
    if(!initialize)
...
//---------------------------------------------------------------------------------------------------------------------------------
void OrcBerserkMobAI::damaged(AnimatedUnitModel* _unitAI, AnimatedUnitModel* _attacker, const UnitAttack* _attack, float& _realDmg)
{
    if(_unitAI)
    {
        //increase damage
        ...
        CCARRAY_FOREACH(_unitAI->_attacks, obj)
        {
            UnitAttack* att = (UnitAttack*)obj;
        
        //for(vector<UnitAttack*>::iterator i = unitAI->attacks.begin(); i < unitAI->attacks.end(); i++)
        //{
            att->damage += att->damage * raiseDamage * 0.01;
            att->damageDelta += att->damageDelta * raiseDamage * 0.01;
        }

        pBattleModel->showMessage(_unitAI, "Raise!", _attack->dmgColor, ccp(_unitAI->getPosition().x, _unitAI->getPosition().y + 200));
    }
}
