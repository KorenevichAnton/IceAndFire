#include "FrostlingWarriorMobAI.h"
#include "Settings.h"
...
//---------------------------------------------------------------------------------------------------------------------------------
FrostlingWarriorMobAI::FrostlingWarriorMobAI():
UnitAI(),
waitAnimation(0),
initialize(false)
{
    typeUnitAI = FROSTLING_WARRIOR_MOB_AI;
}
...
//---------------------------------------------------------------------------------------------------------------------------------
FrostlingWarriorMobAI* FrostlingWarriorMobAI::createTS(BattleModel* _battleModel)
{
    FrostlingWarriorMobAI* pRet = new FrostlingWarriorMobAI();
    if(pRet)
...
//---------------------------------------------------------------------------------------------------------------------------------
void FrostlingWarriorMobAI::init(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits)
{
...
//---------------------------------------------------------------------------------------------------------------------------------
void FrostlingWarriorMobAI::update(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits, float _dt)
{
    if(!initialize)
...
//---------------------------------------------------------------------------------------------------------------------------------
void FrostlingWarriorMobAI::damaged(AnimatedUnitModel* _unitAI, AnimatedUnitModel* _attacker, const UnitAttack* _attack, float& _realDmg)
{
    if(_unitAI)
    {
        //this unit is don't push
        CCObject* obj;
