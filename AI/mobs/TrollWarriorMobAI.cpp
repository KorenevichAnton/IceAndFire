...
//---------------------------------------------------------------------------------------------------------------------------------
TrollWarriorMobAI::TrollWarriorMobAI():
UnitAI(),
waitAnimation(0),
initialize(false),
resist(0)
{
    typeUnitAI = TROLL_WARRIOR_MOB_AI;
}
...
//---------------------------------------------------------------------------------------------------------------------------------
TrollWarriorMobAI* TrollWarriorMobAI::createTS(BattleModel* _battleModel)
{
    TrollWarriorMobAI* pRet = new TrollWarriorMobAI();
    if(pRet)
...
//---------------------------------------------------------------------------------------------------------------------------------
void TrollWarriorMobAI::init(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits)
{
    /* here a initialize default parameters
    */

    resist = _unitAI->getAdditionalParameter(1);    //block magic damage in percent

    initialize = true;
}

//---------------------------------------------------------------------------------------------------------------------------------
void TrollWarriorMobAI::update(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits, float _dt)
{
    if(!initialize)
...
//---------------------------------------------------------------------------------------------------------------------------------
void TrollWarriorMobAI::damaged(AnimatedUnitModel* _unitAI, AnimatedUnitModel* _attacker, const UnitAttack* _attack, float& _realDmg)
{
    if(_unitAI->isAlive())
    {
        if(_attacker->getType() == AUM_TYPE_SPELL)
        {
            _realDmg = _realDmg * resist * 0.01;
            pBattleModel->showMessage(_unitAI, "Resist magic!", _attack->dmgColor, ccp(_unitAI->getPosition().x, _unitAI->getPosition().y + 200));
        }
    }
}
