#include "FrostlingTrapperMobAI.h"
#include "Settings.h"
...
//---------------------------------------------------------------------------------------------------------------------------------
FrostlingTrapperMobAI::FrostlingTrapperMobAI():
UnitAI(),
initialize(false),
panicTime(0),
waitAnimation(0),
panicTimeRemained(0)
{
    typeUnitAI = FROSTLING_TRAPPER_MOB_AI;
}
...
//---------------------------------------------------------------------------------------------------------------------------------
FrostlingTrapperMobAI* FrostlingTrapperMobAI::createTS(BattleModel* _battleModel)
{
    FrostlingTrapperMobAI* pRet = new FrostlingTrapperMobAI();
    if(pRet)
...
//---------------------------------------------------------------------------------------------------------------------------------
void FrostlingTrapperMobAI::init(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits)
{
    /* here a initialize default parameters
    */

    panicTime = _unitAI->getAdditionalParameter(1);  //time of run back

    initialize = true;
}

//---------------------------------------------------------------------------------------------------------------------------------
void FrostlingTrapperMobAI::update(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits, float _dt)
{
    if(!initialize)
...

    {
        ...
        else
        {
            ...
            if(canMove(_unitAI, _allUnits))
            {
                ...
                //process of running back
                if(panicTimeRemained)
                {
                    panicTimeRemained -= _dt;
                    if(panicTimeRemained <= 0)
                    {
                        panicTimeRemained = 0;

                        _unitAI->setMoveTarget( ccp(Settings::sharedInstance()->getBattleFieldRect().size.width - _unitAI->getMoveTarget().x,
                                                                _unitAI->getMoveTarget().y) );
                        (_unitAI->getLookMode() == AUM_LOOK_LEFT_MODE) ? (_unitAI->setLookMode(AUM_LOOK_RIGHT_MODE)) : (_unitAI->setLookMode(AUM_LOOK_LEFT_MODE));
                    }
                }
                //
            }
            else
            {
                ...
                //stop running back because there is no escape
                if(panicTimeRemained)
                {
                    panicTimeRemained = 0;
...
//---------------------------------------------------------------------------------------------------------------------------------
void FrostlingTrapperMobAI::damaged(AnimatedUnitModel* _unitAI, AnimatedUnitModel* _attacker, const UnitAttack* _attack, float& _realDmg)
{
    if(_unitAI)
    {
        //start/continue running back if received melee damage
        if(_attacker->getType() == AUM_TYPE_MOB)
        {
            if(!panicTimeRemained)
            {
                _unitAI->setMoveTarget( ccp(Settings::sharedInstance()->getBattleFieldRect().size.width - _unitAI->getMoveTarget().x,
