...
//---------------------------------------------------------------------------------------------------------------------------------
ZvergStoneMobAI::ZvergStoneMobAI():
UnitAI(),
waitAnimation(0),
initialize(false)
{
    typeUnitAI = ZVERG_STONE_MOB_AI;
}
...
//---------------------------------------------------------------------------------------------------------------------------------
ZvergStoneMobAI* ZvergStoneMobAI::createTS(BattleModel* _battleModel)
{
    ZvergStoneMobAI* pRet = new ZvergStoneMobAI();
    if(pRet)
...
//---------------------------------------------------------------------------------------------------------------------------------
void ZvergStoneMobAI::init(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits)
{
...
//---------------------------------------------------------------------------------------------------------------------------------
void ZvergStoneMobAI::update(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits, float _dt)
{
    if(!initialize)
...
//---------------------------------------------------------------------------------------------------------------------------------
void ZvergStoneMobAI::damaged(AnimatedUnitModel* _unitAI, AnimatedUnitModel* _attacker, const UnitAttack* _attack, float& _realDmg)
{
    if(_unitAI->isAlive())
    {
        if(_attacker->getType() == AUM_TYPE_BULLET)
        {
            if(_attacker->getBehavior()->getTypeUnitAI() != TRAP_BULLET_AI)
            {
                //create reflection bullet
                AnimatedUnitModel* bullet = BulletFactory::createTS(_attacker->getSpecialID(), pBattleModel);
                bullet->setInvoker(_unitAI);
                bullet->setTeam(_unitAI->getTeam());
                bullet->setLine(_unitAI->getLine());
                bullet->setLookMode( (_attacker->getLookMode() == AUM_LOOK_LEFT_MODE) ? AUM_LOOK_RIGHT_MODE : AUM_LOOK_LEFT_MODE );

                cocos2d::CCPoint pos = _attacker->getPosition();
                bullet->setPosition(pos);

                //copy attack stats from attacker
                bullet->getAttack()->damage = _attack->damage;
                bullet->getAttack()->damageDelta = _attack->damageDelta;
                bullet->getAttack()->damageTime = 0;
                bullet->getAttack()->criticalChance = _attack->criticalChance;
                bullet->getAttack()->criticalPower = _attack->criticalPower;
                bullet->getAttack()->schoolName = _attack->schoolName;

            /**  fucking unexplained bug
                //set attack bufs from attacker
                for(vector<Buf*>::const_iterator i = _attack->bufs.begin(); i < _attack->bufs.end(); i++)
                {
                    if((*i)->getKind() == BUF_SEND)
                        bullet->getAttack()->bufs.push_back( (*i)->copy() );
                }
            */

                cmdAddUnit* add = cmdAddUnit::createTS(pBattleModel, bullet, 0);
                ...
                pBattleModel->showMessage(_unitAI, "Mirror!", _attack->dmgColor, ccp(_unitAI->getPosition().x, _unitAI->getPosition().y + 200));
            }
        }
    }
}
