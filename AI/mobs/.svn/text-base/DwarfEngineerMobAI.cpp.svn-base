...
//---------------------------------------------------------------------------------------------------------------------------------
DwarfEngineerMobAI::DwarfEngineerMobAI():
UnitAI(),
initialize(false),
waitAnimation(0),
summonOffset(0)
{
    typeUnitAI = DWARF_ENGINEER_MOB_AI;
}
...
//---------------------------------------------------------------------------------------------------------------------------------
DwarfEngineerMobAI* DwarfEngineerMobAI::createTS(BattleModel* _battleModel)
{
    DwarfEngineerMobAI* pRet = new DwarfEngineerMobAI();
    if(pRet)
...
//---------------------------------------------------------------------------------------------------------------------------------
void DwarfEngineerMobAI::init(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits)
{
    /* here a initialize default parameters
    */

    summonOffset = 50;  //distance from invoker to the summon trap point

    initialize = true;
}

//---------------------------------------------------------------------------------------------------------------------------------
void DwarfEngineerMobAI::update(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits, float _dt)
{
    if(!initialize)
