...
//---------------------------------------------------------------------------------------------------------------------------------
OrcShamanMobAI::OrcShamanMobAI():
UnitAI(),
waitAnimation(0),
initialize(false)
{
    typeUnitAI = ORC_SHAMAN_MOB_AI;
}
...
//---------------------------------------------------------------------------------------------------------------------------------
OrcShamanMobAI* OrcShamanMobAI::createTS(BattleModel* _battleModel)
{
    OrcShamanMobAI* pRet = new OrcShamanMobAI();
    if(pRet)
...
//---------------------------------------------------------------------------------------------------------------------------------
void OrcShamanMobAI::init(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits)
{
...
//---------------------------------------------------------------------------------------------------------------------------------
void OrcShamanMobAI::update(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits, float _dt)
{
    if(!initialize)
...
//---------------------------------------------------------------------------------------------------------------------------------
AnimatedUnitModel* OrcShamanMobAI::getFriendly(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits, int _priority)
{
    ...
    //for(vector<AnimatedUnitModel*>::const_iterator i = _allUnits.begin();  i < _allUnits.end(); i++)
    //{
        if( unit->isAlive() &&
            unit != _unitAI &&
            ...
            unit->getLine() == _unitAI->getLine()*/ )
        {
            if(isBodyCircleInRange(_unitAI, unit, _priority))
            {
                ...
                {
                    if(_unitAI->getAttack(_priority)->subtype == "buf")
                    {
                        if(!unit->issetBuf(BUF_REGENERATION))
                        {
