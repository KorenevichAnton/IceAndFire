...
//---------------------------------------------------------------------------------------------------------------------------------
MehanicKillerMobAI::MehanicKillerMobAI():
UnitAI(),
waitAnimation(0),
initialize(false)
{
    typeUnitAI = MEHANIC_KILLER_MOB_AI;
}
...
//---------------------------------------------------------------------------------------------------------------------------------
MehanicKillerMobAI* MehanicKillerMobAI::createTS(BattleModel* _battleModel)
{
    MehanicKillerMobAI* pRet = new MehanicKillerMobAI();
    if(pRet)
...
//---------------------------------------------------------------------------------------------------------------------------------
void MehanicKillerMobAI::init(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits)
{
...
//---------------------------------------------------------------------------------------------------------------------------------
void MehanicKillerMobAI::update(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits, float _dt)
{
    if(!initialize)
                    ...
                    if(att->subtype == "melee")
                    {
                        ...
                        CCARRAY_FOREACH(_allUnits, obj)
                        {
                            ...
                            if( unit->isAlive() &&
                                ...
                                unit != _unitAI->getCurAttackTarget() &&
                                unit->getTeam() != _unitAI->getTeam() &&
                                unit->getLine() == _unitAI->getLine() &&
                                //check  for membership in splash radius
                                unit->getPosition().x > _unitAI->getPosition().x - att->range &&
                                unit->getPosition().x < _unitAI->getPosition().x + att->range )
                            {
                                cmdAttack* cmdAtt = cmdAttack::createTS(pBattleModel, _unitAI, unit, att, att->damageTime);
                                pBattleModel->addCommand(cmdAtt);
                                cmdAtt->release();
                            }
                        }
