...
//---------------------------------------------------------------------------------------------------------------------------------
ZvergWarriorMobAI::ZvergWarriorMobAI():
UnitAI(),
waitAnimation(0),
initialize(false)
{
    typeUnitAI = ZVERG_WARRIOR_MOB_AI;
}
...
//---------------------------------------------------------------------------------------------------------------------------------
ZvergWarriorMobAI* ZvergWarriorMobAI::createTS(BattleModel* _battleModel)
{
    ZvergWarriorMobAI* pRet = new ZvergWarriorMobAI();
    if(pRet)
...
//---------------------------------------------------------------------------------------------------------------------------------
void ZvergWarriorMobAI::init(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits)
{
...
//---------------------------------------------------------------------------------------------------------------------------------
void ZvergWarriorMobAI::update(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits, float _dt)
{
    if(!initialize)
...
//---------------------------------------------------------------------------------------------------------------------------------
void ZvergWarriorMobAI::damaged(AnimatedUnitModel* _unitAI, AnimatedUnitModel* _attacker, const UnitAttack* _attack, float& _realDmg)
{
    if(_unitAI->isAlive())
    {
        if(_attacker->getType() == AUM_TYPE_MOB)
        {
            //create reflected attack
            cmdAttack* cmdAtt = cmdAttack::createTS(pBattleModel, _attacker, _attacker, _attacker->getAttack(_attack->priority), 0);
            pBattleModel->addCommand(cmdAtt);
            cmdAtt->release();

            pBattleModel->showMessage(_unitAI, "Reflected!", _attack->dmgColor, ccp(_unitAI->getPosition().x, _unitAI->getPosition().y + 200));
        }

        if(_attacker->getType() == AUM_TYPE_BULLET)
        {
            //create reflected attack
            cmdAttack* cmdAtt = cmdAttack::createTS(pBattleModel, _attacker, _attacker->getInvoker(), _attacker->getAttack(_attack->priority), 0);
            pBattleModel->addCommand(cmdAtt);
            cmdAtt->release();
