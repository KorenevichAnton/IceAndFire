...
//---------------------------------------------------------------------------------------------------------------------------------
DwarfWarriorMobAI::DwarfWarriorMobAI():
UnitAI(),
waitAnimation(0),
initialize(false),
blockChance(0)
{
    typeUnitAI = DWARF_WARRIOR_MOB_AI;
}
...
//---------------------------------------------------------------------------------------------------------------------------------
DwarfWarriorMobAI* DwarfWarriorMobAI::createTS(BattleModel* _battleModel)
{
    DwarfWarriorMobAI* pRet = new DwarfWarriorMobAI();
    if(pRet)
...
//---------------------------------------------------------------------------------------------------------------------------------
void DwarfWarriorMobAI::init(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits)
{
...
//---------------------------------------------------------------------------------------------------------------------------------
void DwarfWarriorMobAI::update(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits, float _dt)
{
    if(!initialize)
...
//---------------------------------------------------------------------------------------------------------------------------------
void DwarfWarriorMobAI::damaged(AnimatedUnitModel* _unitAI, AnimatedUnitModel* _attacker, const UnitAttack* _attack, float& _realDmg)
{
    ...
    if(_unitAI)
    {
        ...
        {
            _realDmg = 0;
            pBattleModel->showMessage(_unitAI, "Magic immunity!", _attack->dmgColor, ccp(_unitAI->getPosition().x, _unitAI->getPosition().y + 200));
        }
