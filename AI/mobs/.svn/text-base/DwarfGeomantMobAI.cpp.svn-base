...
//---------------------------------------------------------------------------------------------------------------------------------
DwarfGeomantMobAI::DwarfGeomantMobAI():
UnitAI(),
waitAnimation(0),
initialize(false)
{
    typeUnitAI= DWARF_GEOMANT_MOB_AI;
}
...
//---------------------------------------------------------------------------------------------------------------------------------
DwarfGeomantMobAI* DwarfGeomantMobAI::createTS(BattleModel* _battleModel)
{
    DwarfGeomantMobAI* pRet = new DwarfGeomantMobAI();
    if(pRet)
...
//---------------------------------------------------------------------------------------------------------------------------------
void DwarfGeomantMobAI::init(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits)
{
...
//---------------------------------------------------------------------------------------------------------------------------------
void DwarfGeomantMobAI::update(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits, float _dt)
{
    if(!initialize)
...
//---------------------------------------------------------------------------------------------------------------------------------
AnimatedUnitModel* DwarfGeomantMobAI::getFriendly(AnimatedUnitModel* _unitAI, const cocos2d::CCArray* _allUnits, int _priority)
{
            ...
            unit->getLine() == _unitAI->getLine()*/ )
        {
            if(isBodyCircleInRange(_unitAI, unit, _priority))
            {
                if(isBodyPolygonInRange(_unitAI, unit, _priority))
                {
                    ...
                    {
                        if(!unit->issetBuf(BUF_LIFE_POWER))
                        {
                            target = unit;
                            break;
                        }
